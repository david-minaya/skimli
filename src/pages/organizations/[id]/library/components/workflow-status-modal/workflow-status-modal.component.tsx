    ...
    <Dialog
      ...
      <DialogContent sx={style.content}>
        ...
        <Box sx={style.sectionContent}>
          ...
          <Box sx={style.item}>
            ...
            <Box sx={style.itemTag}>
              {asset.status === 'CONVERTING' && t('workflowStatusModal.convertingStatus')}
              {asset.status === 'CONVERTED' && t('workflowStatusModal.convertedStatus')}
              {asset.status === 'ERRORED' && t('workflowStatusModal.errorStatus')}
            </Box>
          </Box>
          <Box sx={style.item}>
            <Box sx={style.itemTitle}>{t('workflowStatusModal.activity')}</Box>
            <Box sx={style.itemTag}>
              {asset.activityStatus.charAt(0).toUpperCase() + asset.activityStatus.slice(1).toLowerCase()}
            </Box>
          </Box>
          <Box sx={style.item}>
            <Box sx={style.itemTitle}>{t('workflowStatusModal.timeSubmitted')}</Box>
            <Box sx={style.itemText}>{formatDate(asset.activityStartTime)}</Box>
          </Box>
          <Box sx={style.item}>
            <Box sx={style.itemTitle}>{t('workflowStatusModal.timeStarted')}</Box>
            <Box sx={style.itemText}>{formatDate(asset.activityStartTime)}</Box>
          </Box>
          <Box sx={style.item}>
            <Box sx={style.itemTitle}>{t('workflowStatusModal.timeElapsed')}</Box>
            <Box sx={style.itemElapsedTime}>{calcElapsedTime()}</Box>
          </Box>
        </Box>
        ...
        <Box sx={style.sectionContent}>
          ...
          <Box sx={style.item}>
            <Box sx={style.itemTitle}>{t('workflowStatusModal.sampleRatio')}</Box>
