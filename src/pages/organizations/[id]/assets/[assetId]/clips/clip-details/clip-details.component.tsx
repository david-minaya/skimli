...
import { AudioTrackTimeline } from "../audio-track-timeline/audio-track-timeline.component";
...
export function ClipDetails(props: Props) {
  ...
  const timelineAudio = Assets.getTimelineAudio(asset.uuid);
  ...
  async function handleEditClip(clip: Clip) {
    ...
    try {
      const response = await adjustClip(
        clip.uuid,
        asset.uuid,
        toTime(clip.startTime),
        toTime(clip.endTime)
  ...
  function isDuplicated(clip: Clip) {
    ...
    const duplicated = asset.inferenceData?.human.clips.find(
      (clipItem) =>
          ...
          <Box sx={style.center}>
            ...
            <Box sx={style.videoContainer}>
              ...
              <LinearProgress
                variant="determinate"
                value={
                  ((videoPlayer.currentTime - clip.startTime) * 100) /
                  clip.duration
                }
              />
            ...
            <Box sx={style.info}>
              ...
              <OutlinedButton sx={style.resetButton} title={t("resetButton")} />
              <OutlinedButton
