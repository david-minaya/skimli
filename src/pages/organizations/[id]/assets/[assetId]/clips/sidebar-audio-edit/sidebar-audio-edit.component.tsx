...
export function SidebarAudioEdit(props: Props) {
  ...
  function trimAndLengthValidator(trim?: number) {
    if (trim === undefined || isNaN(trim) || trim > timelineAudio!.sources!.duration!) {
      return `Value must be between 0 and ${Math.round(timelineAudio!.sources!.duration!)}`;
    }
  }
      ...
      {timelineAudio &&
        <Fragment>
          <Box sx={style.titleContainer}>
            ...
            <Box sx={style.duration}>&nbsp;- {formatSeconds(timelineAudio!.sources?.duration || 0)}</Box>
          </Box>
          <Box sx={style.fieldTitle}>Volume</Box>
          ...
          <TextField
            ...
            min={0}
            max={Math.round(timelineAudio.sources?.duration || 0)}
            errorMessage={trimAndLengthValidator(Math.round(timelineAudio.length))}
